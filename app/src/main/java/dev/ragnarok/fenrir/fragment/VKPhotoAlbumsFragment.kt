...
import dev.ragnarok.fenrir.mvp.presenter.PhotoAlbumsPresenter.AdditionalParams
...
class VKPhotoAlbumsFragment : BaseMvpFragment<PhotoAlbumsPresenter, IPhotoAlbumsView>(),
    ...
    override fun openAlbum(accountId: Int, album: PhotoAlbum, owner: Owner?, action: String?) {
        if (album.id == -311) {
            ...
            getVKPhotosAlbumPlace(accountId, album.ownerId, album.id, action)
    ...
    override fun doSelection(album: PhotoAlbum) {
        ...
        result.putExtra(Extra.ALBUM_ID, album.id)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.action_photo_comments) {
            presenter?.fireAllComments()
            return true
        }
        if (item.itemId == R.id.action_photo_toggle_like) {
