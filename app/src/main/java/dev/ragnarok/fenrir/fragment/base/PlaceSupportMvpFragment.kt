...
abstract class PlaceSupportMvpFragment<P : PlaceSupportPresenter<V>, V> : BaseMvpFragment<P, V>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        parentFragmentManager.setFragmentResultListener(
            ...
            when (method) {
                ...
                PostShareDialog.Methods.SEND_MESSAGE -> SendAttachmentsActivity.startForSendAttachments(
                    requireActivity(),
                    accountId1,
                    post1
                )
    ...
    override fun openPhotoAlbum(accountId: Int, album: PhotoAlbum) {
        PlaceFactory.getVKPhotosAlbumPlace(accountId, album.ownerId, album.id, null)
            .tryOpenWith(requireActivity())
    }
    ...
    override fun openWikiPage(accountId: Int, page: WikiPage) {
        PlaceFactory.getExternalLinkPlace(accountId, page.viewUrl)
            .tryOpenWith(requireActivity())
    }

    override fun toMarketAlbumOpen(accountId: Int, market_album: MarketAlbum) {
        PlaceFactory.getMarketPlace(accountId, market_album.owner_id, market_album.id)
            .tryOpenWith(requireActivity())
    }

    override fun toArtistOpen(accountId: Int, artist: AudioArtist) {
        PlaceFactory.getArtistPlace(accountId, artist.id, false).tryOpenWith(requireActivity())
    }
