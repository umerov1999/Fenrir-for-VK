...
class GroupWallFragment : AbsWallFragment<IGroupWallView, GroupWallPresenter>(), IGroupWallView {
    ...
    override fun displayBaseCommunityData(community: Community, details: CommunityDetails) {
        ...
        if (details.cover != null && details.cover.images.nonNullNoEmpty()) {
            var def = 0
            var url: String? = null
            for (i in details.cover.images) {
        ...
        val screenName =
            if (community.screenName.nonNullNoEmpty()) "@" + community.screenName else null
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.action_community_control) {
            presenter?.fireCommunityControlClick()
            return true
        }
        if (item.itemId == R.id.action_community_messages) {
            presenter?.fireCommunityMessagesClick()
            return true
        }
        if (item.itemId == R.id.action_show_qr) {
    ...
    private inner class GroupHeaderHolder(root: View) {
        ...
        val tvScreenName: TextView = root.findViewById(R.id.header_group_id)
