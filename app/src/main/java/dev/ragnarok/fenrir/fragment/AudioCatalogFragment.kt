...
class AudioCatalogFragment : BaseMvpFragment<AudioCatalogPresenter, IAudioCatalogView>(),
    IAudioCatalogView, AudioCatalogAdapter.ClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setHasOptionsMenu(
            requireArguments().containsKey(Extra.ARTIST) && !requireArguments().getString(
                Extra.ARTIST
            )
                .isNullOrEmpty()
        )
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_audio_catalog, container, false)
    ...
    override fun onClick(index: Int, value: AudioCatalog) {
        if (!value.audios.isNullOrEmpty()) {
            getAudiosInCatalogBlock(
                presenter?.accountId ?: Settings.get().accounts().current, value.id, value.title
            ).tryOpenWith(requireActivity())
        } else if (!value.playlists.isNullOrEmpty()) {
            getPlaylistsInCatalogBlock(
                presenter?.accountId ?: Settings.get().accounts().current, value.id, value.title
            ).tryOpenWith(requireActivity())
        } else if (!value.videos.isNullOrEmpty()) {
            getVideosInCatalogBlock(
                presenter?.accountId ?: Settings.get().accounts().current, value.id, value.title
            ).tryOpenWith(requireActivity())
        } else if (!value.links.isNullOrEmpty()) {
