...
object Utils {
    ...
    @JvmStatic
    fun <T> cloneListAsArrayList(original: List<T>?): ArrayList<T>? {
        if (original == null) {
            return null
        }
    ...
    @JvmStatic
    fun isEmpty(body: CharSequence?): Boolean {
        return body.isNullOrEmpty()
    }

    @JvmStatic
    fun nonEmpty(text: CharSequence?): Boolean {
        return text.nonNullNoEmpty()
    }

    @JvmStatic
    fun nonEmpty(data: Collection<*>?): Boolean {
        return data.nonNullNoEmpty()
    }
    ...
    @JvmStatic
    fun isEmpty(mids: IntArray?): Boolean {
        return mids.nullOrEmpty()
    }

    @JvmStatic
    fun isEmpty(array: SparseArray<*>?): Boolean {
    ...
    fun getExoPlayerFactory(
        ...
        return OkHttpDataSource.Factory(builder.build() as Call.Factory).setUserAgent(userAgent)
    }
    ...
    @JvmStatic
    fun <T> isValueAssigned(value: T, args: Array<T>): Boolean {
        return listOf(*args).contains(value)
    }

    @JvmStatic
    fun <T> isValueAssigned(value: T, args: List<T>): Boolean {
        return args.contains(value)
    }

    @JvmStatic
    fun <T> isOneElementAssigned(array: List<T>, args: Array<T>): Boolean {
    ...
    @JvmStatic
    fun <T> isOneElementAssigned(array: List<T>, args: List<T>): Boolean {
        for (i in array) {
            if (args.contains(i)) {
                return true
            }
        ...
        builder.build().newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    val resp = (response.body ?: return).string()
    ...
    fun <T> getRandomFromArray(list: Array<T>?): T? {
        return if (list.isNullOrEmpty()) {
            null
        } else {
            list[Random().nextInt(list.size)]
    ...
    fun nextIntInRangeButExclude(start: Int, end: Int, excludes: Array<Int>?): Int {
        if (excludes.isNullOrEmpty()) {
            return rnd(start, end)
        }
        val ints: MutableList<Int> = ArrayList(end - start + 1)
        for (i in start..end) {
            ints.add(i)
        }
        ints.removeAll(listOf(*excludes))
        val ret = getRandomFromList(ints)
        return ret ?: -1
    }
