...
abstract class AccountDependencyDialogFragment : BaseDialogFragment(), OnAttachmentsActionCallback {
    ...
    override fun onGoToMessagesLookup(message: Message) {
        getMessagesLookupPlace(accountId, message.peerId, message.id, null).tryOpenWith(
            requireActivity()
        )
    }
    ...
    override fun onWikiPageOpen(page: WikiPage) {
        getExternalLinkPlace(accountId, page.viewUrl).tryOpenWith(requireActivity())
    }
    ...
    override fun onMarketAlbumOpen(market_album: MarketAlbum) {
        getMarketPlace(accountId, market_album.owner_id, market_album.id).tryOpenWith(
            requireActivity()
        )
    }
    ...
    override fun onArtistOpen(artist: AudioArtist) {
        getArtistPlace(accountId, artist.id, false).tryOpenWith(requireActivity())
    }
