...
class PrivacyViewFragment : AccountDependencyDialogFragment(), PrivacyAdapter.ActionListener {
    ...
    override fun onTypeClick() {
        ...
        MaterialAlertDialogBuilder(requireActivity())
            .setItems(items) { _: DialogInterface?, which: Int ->
                when (which) {
                    0 -> mPrivacy?.type = Privacy.Type.ALL
                    1 -> mPrivacy?.type = Privacy.Type.FRIENDS
                    2 -> mPrivacy?.type = Privacy.Type.FRIENDS_OF_FRIENDS
                    3 -> mPrivacy?.type = Privacy.Type.ONLY_ME
                }
                safeNotifyDatasetChanged()
    ...
    companion object {
        ...
        fun newInstance(args: Bundle?): PrivacyViewFragment {
            val fragment = PrivacyViewFragment()
