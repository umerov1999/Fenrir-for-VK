...
class ChatMembersListAdapter(context: Context, private var data: List<AppChatUser>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item.isOwner) {
            ...
            holder.tvInvitedBy.text = context.getString(R.string.invited_by, item.inviter.fullName)
            if (item.join_date > 0) {
        ...
        if (user.domain.nonNullNoEmpty()) {
            holder.tvDomain.text = "@" + user.domain
        } else {
            holder.tvDomain.text = "@id" + user.ownerId
        }
        ...
        override fun onCreateContextMenu(menu: ContextMenu, v: View, menuInfo: ContextMenuInfo?) {
            ...
            if (isOwner && !item.isOwner) {
                menu.add(
                    ...
                    actionListener?.onAdminToggleClick(!item.isAdmin, item.id)
                    true
                }
            }
        }
