...
class MessagesAdapter(
    ...
    private fun bindStickerHolder(holder: StickerMessageHolder, message: Message, position: Int) {
        ...
        val sticker = message.attachments.stickers[0]
        ...
        val hasAttachments =
            message.fwd.nonNullNoEmpty() || message.attachments != null && message.attachments.size_no_stickers() > 0
    ...
    private fun bindGiftHolder(holder: GiftMessageHolder, message: Message, position: Int) {
        ...
        val giftItem = message.attachments.gifts[0]
        with()
            .load(giftItem.thumb256)
            .into(holder.gift)
    }
    ...
    private fun bindBaseMessageHolder(holder: BaseMessageHolder, message: Message, position: Int) {
        ...
        var read =
            if (message.isOut) lastReadId.outgoing >= message.id else lastReadId.incoming >= message.id
        ...
        if (message.isSelected) {
            ...
            val avaurl = if (message.sender != null) message.sender.maxSquareAvatar else null
    ...
    private fun bindNormalMessage(holder: MessageHolder, message: Message, position: Int) {
        bindBaseMessageHolder(holder, message, position)
        if (holder.botKeyboardView != null && message.keyboard != null && message.keyboard.inline && message.keyboard.buttons.size > 0) {
        ...
        val hasAttachments =
            message.fwd.nonNullNoEmpty() || message.attachments != null && message.attachments.size() > 0
        holder.attachmentsRoot.visibility = if (hasAttachments) View.VISIBLE else View.GONE
        if (hasAttachments) {
            attachmentsViewBinder.displayAttachments(
                message.attachments,
    ...
    private fun bindServiceHolder(holder: ServiceMessageHolder, message: Message, position: Int) {
        ...
        if (message.keyboard != null && message.keyboard.inline && message.keyboard.buttons.size > 0) {
            holder.botKeyboardView.visibility = View.VISIBLE
            holder.botKeyboardView.setButtons(message.keyboard.buttons, false)
        } else {
            holder.botKeyboardView.visibility = View.GONE
