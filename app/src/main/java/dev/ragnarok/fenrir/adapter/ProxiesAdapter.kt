...
class ProxiesAdapter(data: MutableList<ProxyConfig>, private val actionListener: ActionListener) :
    ...
    override fun onBindItemViewHolder(viewHolder: Holder, position: Int, type: Int) {
        ...
        viewHolder.address.text = config.address
        viewHolder.port.text = config.port.toString()
        viewHolder.username.text = config.user
        val pass = StringBuilder()
        if (config.pass.nonNullNoEmpty()) {
            for (i in 0 until config.pass.length) {
                pass.append("*")
            }
        }
