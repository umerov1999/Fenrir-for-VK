...
class AccountAdapter(
    ...
    private fun getDataTypeByAdapterPosition(adapterPosition: Int): Int {
        return if (Utils.isHiddenAccount(getByPosition(adapterPosition).id) && !showHidden) {
            DATA_TYPE_HIDDEN
    ...
    override fun onBindViewHolder(dualHolder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (owner == null) {
            holder.firstName.text = account.id.toString()
        ...
        if (owner is User) {
            ...
            if (owner.domain.nonNullNoEmpty()) {
                ...
                holder.domain.text = "@id" + account.id
            }
            ...
            holder.domain.text = "club" + abs(account.id)
            holder.tvLastTime.visibility = View.GONE
            holder.vOnline.visibility = View.GONE
        }
        val isCurrent = account.id == Settings.get()
            .accounts()
            .current
