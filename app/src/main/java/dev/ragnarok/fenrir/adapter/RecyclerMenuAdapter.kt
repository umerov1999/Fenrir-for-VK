...
import dev.ragnarok.fenrir.adapter.RecyclerMenuAdapter.MenuItemHolder
...
class RecyclerMenuAdapter : RecyclerView.Adapter<MenuItemHolder> {
    ...
    private fun onBindMenuItemHolder(holder: MenuItemHolder, position: Int) {
        ...
        if (headerVisible) {
            ...
            if (section.icon != null) {
                ...
                holder.headerIcon.setImageResource(section.icon)
            } else {
        ...
        bindIcon(holder.itemIcon, item.icon)
        ...
        holder.itemSubtitle.text =
            if (item.subtitle == null) null else item.subtitle.getText(context)
    ...
    private fun bindIcon(imageView: ColorFilterImageView, icon: Icon) {
        ...
        if (icon.isRemote) {
            ...
            imageView.setImageResource(icon.res)
        }
    }
