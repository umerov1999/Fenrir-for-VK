...
import dev.ragnarok.fenrir.adapter.CommunitiesAdapter.Companion.getCommunityType
...
class PeopleAdapter(private val mContext: Context, private var mData: List<Owner>) :
    ...
    private fun bindUserHolder(holder: PeopleHolder, user: User) {
        ...
        displayAvatar(holder.avatar, transformation, avaUrl, Constants.PICASSO_TAG)
        holder.itemView.setOnClickListener {
            mClickListener?.onOwnerClick(user)
        }
        holder.itemView.setOnLongClickListener {
            longClickListener?.onOwnerLongClick(user) == true
        }
    ...
    fun setLongClickListener(longClickListener: LongClickListener?): PeopleAdapter {
