...
class PrivacyAdapter(private val mContext: Context, private val mPrivacy: Privacy) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (position <= mPrivacy.allowedUsers.size) {
            val allowedUserIndex = position - 1
            bindUserEntry(holder as EntryViewHolder, mPrivacy.allowedUsers[allowedUserIndex], true)
            return
        }
        if (position <= count(mPrivacy.allowedUsers, mPrivacy.allowedLists)) {
            val allowedListIndex = position - count(mPrivacy.allowedUsers) - 1
            bindListEntry(holder as EntryViewHolder, mPrivacy.allowedLists[allowedListIndex], true)
            return
        }
        if (position <= count(
                mPrivacy.allowedUsers,
                mPrivacy.allowedLists,
                mPrivacy.disallowedUsers
            ) + 1
        ) {
            val excludedUserIndex =
                position - count(mPrivacy.allowedUsers, mPrivacy.allowedLists) - 2
            bindUserEntry(
                holder as EntryViewHolder,
                mPrivacy.disallowedUsers[excludedUserIndex],
                false
            )
            return
        }
        ...
        bindListEntry(holder as EntryViewHolder, mPrivacy.disallowedLists[excludedListIndex], false)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return if (position == count(mPrivacy.allowedUsers, mPrivacy.allowedLists) + 1) {
            TYPE_TITLE
        } else TYPE_ENTRY
    }
