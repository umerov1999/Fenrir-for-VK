...
class AnswerVKOfficialAdapter(
    private var data: AnswerVKOfficialList?,
    private val context: Context
) : RecyclerView.Adapter<AnswerVKOfficialAdapter.Holder>() {
    ...
    fun getByPosition(position: Int): AnswerVKOfficial? {
    ...
    private fun LoadIcon(holder: Holder, Page: AnswerVKOfficial, isSmall: Boolean) {
        if (!isSmall) {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        val dt = data ?: return
        val Page = dt.items[position]
        val previous = if (position == 0) null else dt.items[position - 1]
        ...
        holder.small.visibility = View.INVISIBLE
        if (Page.header.nonNullNoEmpty()) {
            holder.name.visibility = View.VISIBLE
            val replace = SpannableStringBuilder(fromHtml(Page.header))
            holder.name.setText(
                LinkParser.parseLinks(context, replace),
                TextView.BufferType.SPANNABLE
            )
            val matcher = LinkParser.MENTIONS_AVATAR_PATTERN.matcher(Page.header)
            if (matcher.find()) {
        ...
        if (Page.text.nonNullNoEmpty()) {
            holder.description.visibility = View.VISIBLE
            val replace = SpannableStringBuilder(fromHtml(Page.text))
            holder.description.setText(
                LinkParser.parseLinks(context, replace),
                TextView.BufferType.SPANNABLE
            )
        } else holder.description.visibility = View.GONE
        if (Page.footer.nonNullNoEmpty()) {
            holder.footer.visibility = View.VISIBLE
            val replace = SpannableStringBuilder(fromHtml(Page.footer))
            holder.footer.setText(
                LinkParser.parseLinks(context, replace),
                TextView.BufferType.SPANNABLE
            )
        } else holder.footer.visibility = View.GONE
        ...
        when {
            ...
            Page.attachments.size == 1 -> {
                ...
                holder.attachments.adapter = null
                with()
                    .load(Page.attachments[0].getUrlForSize(PhotoSize.X, false))
                    .tag(Constants.PICASSO_TAG)
                    .placeholder(R.drawable.background_gray)
                ...
                val adapter = FeedbackPhotosAdapter(Page.attachments)
        ...
        if (Page.action != null) {
            if (Page.action.type == AnswerVKOfficial.Action_Types.URL) {
    ...
    fun setData(data: AnswerVKOfficialList) {
        this.data = data
    ...
    interface ClickListener {
        ...
        fun openAction(action: AnswerVKOfficial.Action)
    }
