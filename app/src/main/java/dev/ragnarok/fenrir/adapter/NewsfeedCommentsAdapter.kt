...
import dev.ragnarok.fenrir.adapter.NewsfeedCommentsAdapter.AbsHolder
...
class NewsfeedCommentsAdapter(
    ...
    private fun bindTopic(holder: TopicHolder, position: Int) {
        ...
        if (topic.creator != null) {
            ...
            displayAvatar(
                ...
                transformation,
                topic.creator.get100photoOrSmaller(),
                Constants.PICASSO_TAG
            )
        } else {
    ...
    private fun bindPost(holder: PostHolder, position: Int) {
        ...
        holder.buttonShowMore.visibility =
            if (post.hasText() && post.text.length > 400) View.VISIBLE else View.GONE
        ...
        if (post.creator != null) {
            holder.signerName.text = post.creator.fullName
            displayAvatar(
                holder.signerAvatar,
                transformation,
                post.creator.photo50,
                Constants.PICASSO_TAG
            )
        }
    ...
    private fun needToShowTopDivider(post: Post): Boolean {
        ...
        if (post.copyHierarchy.nonNullNoEmpty() && (attachments == null || (attachments.photos.safeAllIsNullOrEmpty() && attachments.videos.safeAllIsNullOrEmpty()))
        ) {
            return true
        }
