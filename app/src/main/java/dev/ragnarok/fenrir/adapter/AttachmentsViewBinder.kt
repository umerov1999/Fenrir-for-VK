...
import dev.ragnarok.fenrir.adapter.AttachmentsHolder.Companion.forCopyPost
...
class AttachmentsViewBinder(
    ...
    private fun bindVoiceHolder(holder: VoiceHolder, voice: VoiceMessage, messageId: Int?) {
        ...
        // can bee NULL/empty
        if (voice.waveform != null && voice.waveform.isNotEmpty()) {
            holder.mWaveFormView.setWaveForm(voice.waveform)
        } else {
        ...
        if (voice.transcript.isNullOrEmpty()) {
            ...
            if (voice.isShowTranscript || expandVoiceTranscript) {
                ...
                holder.TranscriptText.text = voice.transcript
                holder.mDoTranscript.visibility = View.GONE
            } else {
                holder.TranscriptText.visibility = View.GONE
                holder.mDoTranscript.visibility = View.VISIBLE
                holder.mDoTranscript.setOnClickListener {
                    voice.isShowTranscript = true
                    holder.TranscriptText.visibility = View.VISIBLE
                    holder.TranscriptText.text = voice.transcript
                    holder.mDoTranscript.visibility = View.GONE
                }
    ...
    private fun displayStickers(stickers: List<Sticker>?, stickersContainer: ViewGroup?) {
        ...
        val horisontal = image.height < image.width
        val proporsion = image.width.toDouble() / image.height.toDouble()
    ...
    fun displayFriendsPost(users: List<User>?, container: ViewGroup?, layout: Int) {
        ...
        for (g in 0 until container.childCount) {
            ...
            if (g < users.size) {
                ...
                val user = users[g]
                postViewGroup.visibility = View.VISIBLE
                if (user.fullName.isNullOrEmpty()) holder.tvTitle.visibility =
                    View.INVISIBLE else {
    ...
    fun displayForwards(
        ...
        for (g in 0 until fwdContainer.childCount) {
            ...
            if (g < fwds.size) {
                ...
                tvBody.visibility =
                    if (message.body == null || message.body.isEmpty()) View.GONE else View.VISIBLE
                ...
                tvFwds.setOnClickListener {
                    mAttachmentsActionCallback?.onForwardMessagesOpen(
                        message.fwd
                    )
                }
                val ivAvatar = itemView.findViewById<ImageView>(R.id.item_fwd_message_avatar)
                val senderPhotoUrl =
                    if (message.sender == null) null else message.sender.maxSquareAvatar
                displayAvatar(
                    ivAvatar,
                    mAvatarTransformation,
                    senderPhotoUrl,
    ...
    private fun displayDocs(docs: List<DocLink>?, root: ViewGroup?) {
        ...
        for (g in 0 until root.childCount) {
            ...
            if (g < docs.size) {
                ...
                } else if (doc.type == Types.WALL_REPLY) {
                    tvShowMore.visibility = if (subtitle.length > 400) View.VISIBLE else View.GONE
                    tvDetails.visibility = View.GONE
                    tvPostText.visibility = View.VISIBLE
                    tvPostText.text = OwnerLinkSpanFactory.withSpans(
                ...
                } else if (doc.type == Types.NOT_SUPPORTED) {
                    tvShowMore.visibility = View.GONE
                    tvDetails.visibility = View.GONE
                    tvPostText.visibility = View.VISIBLE
                ...
                when (doc.type) {
                    ...
                    Types.AUDIO_PLAYLIST -> if (imageUrl != null) {
                        ivType.visibility = View.VISIBLE
                        ivPhotoT.visibility = View.VISIBLE
                        displayAvatar(ivPhotoT, null, imageUrl, Constants.PICASSO_TAG)
                    ...
                    Types.MARKET_ALBUM -> if (imageUrl != null) {
                        ivType.visibility = View.VISIBLE
                        ivPhotoT.visibility = View.VISIBLE
                        displayAvatar(ivPhotoT, null, imageUrl, Constants.PICASSO_TAG)
                    ...
                    Types.POST -> {
                        ...
                        val hasAttachments =
                            post.attachments != null && post.attachments.size() > 0
                    ...
                    Types.WALL_REPLY -> {
                        ...
                        val hasCommentAttachments =
                            comment.attachments != null && comment.attachments.size() > 0
            ...
            if (g < articles.size) {
                ...
                if (article.url != null) {
                    ...
                    ivButton.setOnClickListener {
                        mAttachmentsActionCallback?.onUrlOpen(
                            article.url
                        )
                    }
                } else {
                ...
                if (photo_url != null) {
                    ...
                    ivPhoto.setOnLongClickListener {
                        val temp = ArrayList(listOf(article.photo))
    ...
    private fun openDocLink(link: DocLink) {
        when (link.type) {
            Types.DOC -> mAttachmentsActionCallback?.onDocPreviewOpen(
                link.attachment as Document
            )
            Types.POST -> mAttachmentsActionCallback?.onPostOpen(link.attachment as Post)
            Types.LINK -> mAttachmentsActionCallback?.onLinkOpen(link.attachment as Link)
            Types.POLL -> mAttachmentsActionCallback?.onPollOpen(link.attachment as Poll)
            Types.WIKI_PAGE -> mAttachmentsActionCallback?.onWikiPageOpen(
                link.attachment as WikiPage
            )
            Types.STORY -> mAttachmentsActionCallback?.onStoryOpen(link.attachment as Story)
            Types.AUDIO_PLAYLIST -> mAttachmentsActionCallback?.onAudioPlaylistOpen(
                link.attachment as AudioPlaylist
            )
            Types.WALL_REPLY -> mAttachmentsActionCallback?.onWallReplyOpen(
                link.attachment as WallReply
            )
            Types.EVENT -> mAttachmentsActionCallback?.onOpenOwner(-1 * abs((link.attachment as Event).id))
            Types.ALBUM -> mAttachmentsActionCallback?.onPhotoAlbumOpen(
                link.attachment as PhotoAlbum
            )
            Types.MARKET_ALBUM -> mAttachmentsActionCallback?.onMarketAlbumOpen(
                link.attachment as MarketAlbum
            )
            Types.MARKET -> mAttachmentsActionCallback?.onMarketOpen(link.attachment as Market)
            Types.ARTIST -> mAttachmentsActionCallback?.onArtistOpen(link.attachment as AudioArtist)
        }
    }
