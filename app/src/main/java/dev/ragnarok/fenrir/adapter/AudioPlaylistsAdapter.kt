...
class AudioPlaylistsAdapter(
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        if (playlist.description.isNullOrEmpty()) holder.description.visibility = View.GONE else {
    ...
    inner class Holder(itemView: View) : RecyclerView.ViewHolder(itemView),
        ...
        override fun onCreateContextMenu(menu: ContextMenu, v: View, menuInfo: ContextMenuInfo?) {
            ...
            val playlist = data[position]
            if (!Utils.isValueAssigned(playlist.id, Settings.get().other().servicePlaylist)) {
                if (Settings.get().accounts().current == playlist.ownerId) {
                    menu.add(0, v.id, 0, R.string.delete)
                        .setOnMenuItemClickListener {
                            clickListener?.onDelete(position, playlist)
                            true
                        }
                    if (playlist.original_access_key.isNullOrEmpty() || playlist.original_id == 0 || playlist.original_owner_id == 0) {
