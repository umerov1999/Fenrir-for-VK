...
object SelectionUtils {
    ...
    fun addSelectionProfileSupport(context: Context?, root: ViewGroup?, mayBeUser: Any) {
        ...
        var canSelect =
            if (criteria?.isPeopleOnly == true) mayBeUser is User else mayBeUser is Owner || mayBeUser is FavePage
        ...
        if (!canSelect) {
            ...
            if (mayBeUser is FavePage && mayBeUser.owner != null) {
                selectionView.setOnClickListener { callack.select(mayBeUser.owner) }
