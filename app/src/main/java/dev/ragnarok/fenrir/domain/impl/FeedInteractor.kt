...
class FeedInteractor(
    ...
    private fun containInWords(rgx: Set<String>?, dto: VKApiNews): Boolean {
        ...
        if (!dto.text.isNullOrEmpty()) {
            for (i in rgx) {
                if (dto.text.lowercase(Locale.getDefault())
        ...
        if (!dto.copy_history.isNullOrEmpty()) {
            for (i in dto.copy_history) {
                if (!i.text.isNullOrEmpty()) {
                    for (s in rgx) {
                        if (i.text.lowercase(Locale.getDefault())
        ...
        return if ("likes" == sourceIds) {
            networker.vkDefault(accountId)
                ...
                .flatMap { response ->
                    ...
                    val dbos: MutableList<NewsEntity> = ArrayList(feed.size)
                    val ownIds = VKOwnIds()
                    for (news in feed) {
                        if (news.source_id == 0) {
                            continue
