    ...
    private fun getCachedDetails(accountId: Int, userId: Int): Single<Optional<UserDetails>> {
        return cache.getUserDetails(accountId, userId)
            .flatMap { optional: Optional<UserDetailsEntity> ->
                ...
                if (entity.relatives.nonNullNoEmpty()) {
                    for (e in entity.relatives) {
                        if (e.id > 0) {
    ...
    override fun handleOnlineChanges(
        ...
        if (offlineUpdates.nonNullNoEmpty()) {
            for (update in offlineUpdates) {
                ...
                patches.add(
                    UserPatch(update.user_id).setOnlineUpdate(
                        UserPatch.Online(
                            false,
                            lastSeeenUnixtime,
                            0
        ...
        for (patch in patches) {
            ...
            if (patch.online != null) {
                update.online = UserUpdate.Online(
                    patch.online.isOnline,
                    patch.online.lastSeen,
                    patch.online.platform
                )
            }
            if (patch.status != null) {
                update.status = UserUpdate.Status(patch.status.status)
            }
            updates.add(update)
        }
