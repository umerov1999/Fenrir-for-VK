...
object Model2Dto {
    ...
    private fun createToken(model: AbsModel): IAttachmentToken {
        ...
        if (model is Photo) {
            return AttachmentsTokenCreator.ofPhoto(model.id, model.ownerId, model.accessKey)
        }
        if (model is Poll) {
        ...
        if (model is PhotoAlbum) {
            return AttachmentsTokenCreator.ofPhotoAlbum(model.id, model.ownerId)
        }
        if (model is Call) {
        ...
        if (model is WallReply) {
            return AttachmentsTokenCreator.ofWallReply(model.id, model.ownerId)
        }
        ...
        if (model is AudioPlaylist) {
            return if (model.original_access_key.isNullOrEmpty() || model.original_id == 0 || model.original_owner_id == 0) {
