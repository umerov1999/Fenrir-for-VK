...
class GroupSettingsInteractor(
    ...
    override fun editManager(
        ...
        return networker.vkDefault(accountId)
            .groups()
            .editManager(groupId, user.id, targetRole, asContact, position, email, phone)
            .andThen(
                Single
                    .fromCallable {
                        val info = ContactInfo(user.id)
            ...
            .flatMap { items ->
                ...
                ownersRepository.findBaseOwnersDataAsBundle(
                    ...
                    .map { bundle ->
                        ...
                        for (u in items) {
                            var admin: User
                            val banInfo = u.banInfo
                            admin = if (banInfo.adminId != 0) {
                                bundle.getById(u.banInfo.adminId) as User
                            } else {
                                // ignore this
                                continue
                            }
                            ...
                            if (u.profile != null) {
                                infos.add(Banned(transformUser(u.profile), admin, info))
                            } else if (u.group != null) {
                                infos.add(Banned(transformCommunity(u.group), admin, info))
                            }
                        }
        ...
        return networker.vkDefault(accountId)
            ...
            .flatMap { items ->
                networker.vkDefault(accountId)
                    .groups()
                    .getById(setOf(groupId), null, null, "contacts")
                    .map<List<Contact>> { communities: List<VKApiCommunity> ->
                    ...
                    .map<List<Manager>> {
                        ...
                        for (user in users) {
                            ...
                            if (contact != null) {
                                manager.setDisplayAsContact(true).contactInfo = transform(contact)
                            }
