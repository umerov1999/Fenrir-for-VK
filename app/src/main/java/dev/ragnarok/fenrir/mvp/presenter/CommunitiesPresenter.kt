...
class CommunitiesPresenter(accountId: Int, private val userId: Int, savedInstanceState: Bundle?) :
    ...
    companion object {
        ...
        private fun isMatchFilter(community: Community, filter: String?): Boolean {
            ...
            if (community.name.nonNullNoEmpty()) {
                val lowername = community.name.lowercase(Locale.getDefault())
            ...
            return community.screenName.nonNullNoEmpty() && community.screenName.lowercase(Locale.getDefault())
                .contains(lower)
        }
