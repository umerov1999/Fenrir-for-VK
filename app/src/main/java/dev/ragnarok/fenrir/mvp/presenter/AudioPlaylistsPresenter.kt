...
class AudioPlaylistsPresenter(accountId: Int, val owner_id: Int, savedInstanceState: Bundle?) :
    ...
    fun onEdit(context: Context, album: AudioPlaylist) {
        ...
        (root.findViewById<View>(R.id.edit_title) as TextInputEditText).setText(album.title)
        (root.findViewById<View>(R.id.edit_description) as TextInputEditText).setText(album.description)
    ...
    private inner class FindPlaylist(disposable: CompositeDisposable) :
        ...
        override fun compare(data: AudioPlaylist, q: String): Boolean {
            return (safeCheck(
                ...
                object : SafeCallCheckInt {
                    override fun check(): Boolean {
                        return data.artist_name.lowercase(Locale.getDefault()).contains(
                            q.lowercase(
                                Locale.getDefault()
                            )
                        )
