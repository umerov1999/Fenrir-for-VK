...
internal class VideoStorage(base: AppStorages) : AbsStorage(base), IVideoStorage {
    ...
    private fun mapVideo(cursor: Cursor): VideoEntity {
        ...
        val video = VideoEntity().set(id, ownerId)
            ...
            .setCanComment(cursor.getInt(cursor.getColumnIndexOrThrow(VideoColumns.CAN_COMMENT)) == 1)
            .setPrivate(cursor.getInt(cursor.getColumnIndexOrThrow(VideoColumns.IS_PRIVATE)) == 1)
            .setFavorite(cursor.getInt(cursor.getColumnIndexOrThrow(VideoColumns.IS_FAVORITE)) == 1)
            ...
            .setMp4link1440(cursor.getString(cursor.getColumnIndexOrThrow(VideoColumns.MP4_1440)))
            .setMp4link2160(cursor.getString(cursor.getColumnIndexOrThrow(VideoColumns.MP4_2160)))
