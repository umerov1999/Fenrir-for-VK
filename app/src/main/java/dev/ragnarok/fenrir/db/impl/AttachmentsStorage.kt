...
internal class AttachmentsStorage(base: AppStorages) : AbsStorage(base), IAttachmentsStorage {
    ...
    override fun getAttachmentsDbosWithIds(
        ...
        return Single.create { emitter: SingleEmitter<List<Pair<Int, Entity>>> ->
            val cursor = createCursor(accountId, attachToType, attachToDbid)
            val dbos: MutableList<Pair<Int, Entity>> = ArrayList(safeCountOf(cursor))
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    if (emitter.isDisposed) {
                        break
                    }
                    val id = cursor.getInt(cursor.getColumnIndexOrThrow(idColumnFor(attachToType)))
                    val type =
                        cursor.getInt(cursor.getColumnIndexOrThrow(typeColumnFor(attachToType)))
                    val json =
                        cursor.getString(cursor.getColumnIndexOrThrow(dataColumnFor(attachToType)))
                    val entity = deserializeDbo(type, json)
    ...
    override fun getAttachmentsDbosSync(
        ...
        val cursor = createCursor(accountId, attachToType, attachToDbid)
        val entities: MutableList<Entity> = ArrayList(safeCountOf(cursor))
        if (cursor != null) {
            while (cursor.moveToNext()) {
                if (cancelable.isOperationCancelled) {
                    break
                }
                val type = cursor.getInt(cursor.getColumnIndexOrThrow(typeColumnFor(attachToType)))
                val json =
                    cursor.getString(cursor.getColumnIndexOrThrow(dataColumnFor(attachToType)))
    ...
    companion object {
        ...
        fun appendAttachOperationWithBackReference(
            ...
            @AttachToType attachToType: Int, attachToBackReferenceIndex: Int, entity: Entity
        ) {
        ...
        fun appendAttachOperationWithStableAttachToId(
            ...
            attachToDbid: Int, entity: Entity
        ): Int {
            val cv = ContentValues()
        ...
        private fun typeColumnFor(@AttachToType type: Int): String {
            when (type) {
                AttachToType.COMMENT -> return CommentsAttachmentsColumns.TYPE
                AttachToType.MESSAGE -> return MessagesAttachmentsColumns.TYPE
                AttachToType.POST -> return WallAttachmentsColumns.TYPE
            }
            throw IllegalArgumentException()
        }
        ...
        private fun serializeDbo(entity: Entity): String {
            return GSON.toJson(entity)
        }

        private fun deserializeDbo(type: Int, json: String): Entity {
            val dboClass = AttachmentsTypes.classForType(type)
