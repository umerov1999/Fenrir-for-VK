...
internal class MessagesStorage(base: AppStorages) : AbsStorage(base), IMessagesStorage {
    override fun insertPeerDbos(
        ...
        return Completable.create { emitter: CompletableEmitter ->
            ...
            if (clearHistory) {
                ...
                val args = arrayOf(
                    peerId.toString(),
                    java.lang.String.valueOf(MessageColumns.DONT_ATTACH),
    ...
    override fun insert(accountId: Int, dbos: List<MessageEntity>): Single<IntArray> {
    ...
    override fun getByCriteria(
        ...
        return Single.create { emitter: SingleEmitter<List<MessageEntity>> ->
            ...
            val dbos = ArrayList<MessageEntity>(safeCountOf(cursor))
            if (cursor != null) {
    ...
    override fun insert(accountId: Int, peerId: Int, patch: MessageEditEntity): Single<Int> {
        return Single.create { emitter: SingleEmitter<Int> ->
            ...
            cv.put(
                ...
                if (patch.extras == null) null else GSON.toJson(patch.extras)
            )
            cv.put(MessageColumns.PAYLOAD, patch.payload)
            cv.put(
                MessageColumns.KEYBOARD,
                if (patch.keyboard == null) null else GSON.toJson(patch.keyboard)
            )
            ...
            if (patch.attachments.nonNullNoEmpty()) {
                val entities = patch.attachments
            ...
            if (patch.forward.nonNullNoEmpty()) {
                for (fwdDbo in patch.forward) {
            ...
            .flatMap { count ->
                Single
                    .create { emitter: SingleEmitter<Int> ->
                        ...
                        if (patch.attachments.nonNullNoEmpty()) {
                            for (entity in patch.attachments) {
    ...
    private fun fullMapDbo(
        ...
        if (withForwardMessages && dbo.forwardCount > 0) {
            ...
            dbo.forwardMessages = fwds
        } else {
            dbo.forwardMessages = null
        }
        return dbo
    }
    ...
    override fun findDraftMessage(accountId: Int, peerId: Int): Maybe<DraftMessage> {
        return Maybe.create { e: MaybeEmitter<DraftMessage> ->
            ...
            if (cursor != null) {
                if (cursor.moveToNext()) {
                    val id = cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns._ID))
                    val body = cursor.getString(cursor.getColumnIndexOrThrow(MessageColumns.BODY))
            ...
            if (message != null) {
                val count = stores.attachments()
                    ...
                    .blockingGet()
                message.attachmentsCount = count ?: 0
                e.onSuccess(message)
            }
            e.onComplete()
        }
    ...
    override fun deleteMessages(accountId: Int, ids: Collection<Int>): Single<Boolean> {
        return Single.create { e: SingleEmitter<Boolean> ->
            ...
            val uri = getMessageContentUriFor(accountId)
            val where = MessageColumns.FULL_ID + " IN(" + TextUtils.join(",", copy) + ")"
    ...
    private fun getForwardMessages(
        ...
        val dbos: MutableList<MessageEntity> = ArrayList(safeCountOf(cursor))
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                //dbo.setRead(true);
                dbo.isOut = dbo.fromId == accountId
    ...
    override fun findFirstUnsentMessage(
        ...
        withAtatchments: Boolean,
        withForwardMessages: Boolean
    ): Single<Optional<Pair<Int, MessageEntity>>> {
        return Single.create { emitter: SingleEmitter<Optional<Pair<Int, MessageEntity>>> ->
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    if (isFirst) {
                        isFirst = false
                        from_peer =
                            cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.PEER_ID))
                    }
    ...
    companion object {
        ...
        private val EXTRAS_TYPE = object : TypeToken<Map<Int?, String?>?>() {}.type
        fun appendDboOperation(
            ...
            cv.put(MessageColumns.EXTRAS, if (dbo.extras == null) null else GSON.toJson(dbo.extras))
            cv.put(MessageColumns.UPDATE_TIME, dbo.updateTime)
            cv.put(MessageColumns.PAYLOAD, dbo.payload)
            cv.put(
                MessageColumns.KEYBOARD,
                if (dbo.keyboard == null) null else GSON.toJson(dbo.keyboard)
            )
            val uri = getMessageContentUriFor(accountId)
            ...
            if (dbo.isHasAttachmens && dbo.attachments != null) {
        ...
        private fun baseMapDbo(cursor: Cursor): MessageEntity {
            @MessageStatus val status =
                cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.STATUS))
            @ChatAction val action =
                cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.ACTION))
            val id = cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns._ID))
            val peerId = cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.PEER_ID))
            val fromId = cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.FROM_ID))
            var extras: HashMap<Int?, String?>? = null
            var keyboard: KeyboardEntity? = null
            val extrasText = cursor.getString(cursor.getColumnIndexOrThrow(MessageColumns.EXTRAS))
            if (extrasText.nonNullNoEmpty()) {
                extras = GSON.fromJson(extrasText, EXTRAS_TYPE)
            }
            val keyboardText =
                cursor.getString(cursor.getColumnIndexOrThrow(MessageColumns.KEYBOARD))
            if (keyboardText.nonNullNoEmpty()) {
                keyboard = GSON.fromJson(keyboardText, KeyboardEntity::class.java)
            }
            return MessageEntity().set(id, peerId, fromId)
                .setEncrypted(cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.ENCRYPTED)) == 1)
                .setStatus(status)
                .setAction(action)
                .setExtras(extras)
                .setBody(cursor.getString(cursor.getColumnIndexOrThrow(MessageColumns.BODY))) //.setRead(cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.READ_STATE)) == 1)
                ...
                .setDeletedForAll(cursor.getInt(cursor.getColumnIndexOrThrow(MessageColumns.DELETED_FOR_ALL)) == 1) //.setTitle(cursor.getString(cursor.getColumnIndexOrThrow(MessageColumns.TITLE)))
