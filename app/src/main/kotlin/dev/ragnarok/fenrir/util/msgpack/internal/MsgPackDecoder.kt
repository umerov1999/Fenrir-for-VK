package dev.ragnarok.fenrir.util.msgpack.internal

import dev.ragnarok.fenrir.util.msgpack.MsgPackConfiguration
import dev.ragnarok.fenrir.util.msgpack.MsgPackNullableDynamicSerializer
import dev.ragnarok.fenrir.util.msgpack.exceptions.MsgPackSerializationException
import dev.ragnarok.fenrir.util.msgpack.stream.MsgPackDataInputBuffer
import dev.ragnarok.fenrir.util.msgpack.types.MsgPackType
import dev.ragnarok.fenrir.util.msgpack.utils.joinToNumber
...
internal class BasicMsgPackDecoder(
    ...
    override fun decodeInline(inlineDescriptor: SerialDescriptor): Decoder {
        if (inlineDecoders.containsKey(inlineDescriptor)) {
            return inlineDecoders[inlineDescriptor]!!(
                InlineDecoderHelper(
                    serializersModule,
                    dataBuffer
                )
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder {
        if (descriptor.kind in arrayOf(StructureKind.CLASS, StructureKind.OBJECT)) {
            if (descriptor.serialName == "dev.ragnarok.fenrir.util.msgpack.extensions.MsgPackExtension") {
