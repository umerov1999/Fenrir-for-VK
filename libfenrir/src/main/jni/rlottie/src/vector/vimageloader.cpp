...
# endif  // _WIN32
    ...
    void moduleFree() {}
    static bool moduleLoad() { return false; }
    ...
    VBitmap load(const char *fileName) const
    {
    ...
    VBitmap load(const char *imageData, size_t len) const
    {
     ...
     */
    static void convertToBGRAPremul(unsigned char *bits, int width, int height)
    {
     ...
     */
    static void convertToBGRA(unsigned char *bits, int width, int height)
    {
        int            pixelCount = width * height;
        unsigned char *pix = bits;
        for (int i = 0; i < pixelCount; i++) {
