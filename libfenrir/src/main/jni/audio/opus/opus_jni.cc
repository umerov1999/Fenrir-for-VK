...
DECODER_FUNC(jint, opusDecode, jlong jDecoder, jlong jTimeUs,
    ...
    auto *decoder = reinterpret_cast<OpusMSDecoder *>(jDecoder);
    const auto *inputBuffer = reinterpret_cast<const uint8_t *>(
    ...
    int sampleCount;
    if (outputFloat) {
        auto *outputBufferData = reinterpret_cast<float *>(
        ...
        auto *outputBufferData = reinterpret_cast<int16_t *>(
                env->GetDirectBufferAddress(jOutputBufferData));
