...
public:
	static int32_t crc32(const std::string& str) {
		...
		auto* pszData = (uint8_t*)str.c_str();
		for (size_t i = 0; i < str.length(); ++i)
			uiCRC32 = ((uiCRC32 >> 8) & 0x00FFFFFF) ^
			uiCRC32_Table[(uiCRC32 ^ (uint32_t)*pszData++) & 0xFF];

		return (int32_t)(uiCRC32 ^ 0xFFFFFFFF);
	}
private:
	static inline const uint32_t uiCRC32_Table[256] = {
