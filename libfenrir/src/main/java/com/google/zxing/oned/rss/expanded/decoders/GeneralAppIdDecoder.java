...
final class GeneralAppIdDecoder {
    ...
    private DecodedNumeric decodeNumeric(int pos) throws FormatException {
        if (pos + 7 > information.getSize()) {
            ...
            if (numeric == 0) {
                return new DecodedNumeric(information.getSize(), DecodedNumeric.FNC1, DecodedNumeric.FNC1);
            }
            return new DecodedNumeric(information.getSize(), numeric - 1, DecodedNumeric.FNC1);
        }
    ...
    DecodedInformation decodeGeneralPurposeField(int pos, String remaining) throws FormatException {
        ...
        if (lastDecoded != null && lastDecoded.isRemaining()) {
            return new DecodedInformation(current.getPosition(),
                    buffer.toString(), lastDecoded.getRemainingValue());
        }
