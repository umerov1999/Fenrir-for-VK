...
public final class MinimalEncoder {
    ...
    static boolean isInC40Shift1Set(char ch) {
        return ch <= 31;
    }

    static boolean isInC40Shift2Set(char ch, int fnc1) {
    ...
    static boolean isInTextShift1Set(char ch) {
        return isInC40Shift1Set(ch);
    }

    static boolean isInTextShift2Set(char ch, int fnc1) {
    ...
    static int getNumberOfC40Words(Input input, int from, boolean c40, int[] characterLength) {
        ...
        for (int i = from; i < input.length(); i++) {
            ...
            if (c40 ? HighLevelEncoder.isNativeC40(ci) : HighLevelEncoder.isNativeText(ci)) {
                thirdsCount++; //native
            } else if (!isExtendedASCII(ci, input.getFNC1Character())) {
                thirdsCount += 2; //shift
            } else {
                int asciiValue = ci & 0xff;
                if (asciiValue >= 128 && (c40 ? HighLevelEncoder.isNativeC40((char) (asciiValue - 128)) : HighLevelEncoder.isNativeText((char) (asciiValue - 128)))) {
    ...
    private static final class Edge {
        ...
        final Input input;
        final Mode mode; //the mode at the start of this edge.
        ...
        static int getShiftValue(char c, boolean c40, int fnc1) {
            return (c40 ? isInC40Shift1Set(c) : isInTextShift1Set(c)) ? 0 :
                    (c40 ? isInC40Shift2Set(c, fnc1) : isInTextShift2Set(c, fnc1)) ? 1 : 2;
        }
        ...
        byte[] getC40Words(boolean c40, int fnc1) {
            ...
            for (int i = 0; i < characterLength; i++) {
                ...
                if (c40 ? HighLevelEncoder.isNativeC40(ci) : HighLevelEncoder.isNativeText(ci)) {
                    ...
                    if (c40 ? HighLevelEncoder.isNativeC40(asciiValue) : HighLevelEncoder.isNativeText(asciiValue)) {
    ...
    private static final class Input extends MinimalECIInput {
        ...
        Input(String stringToEncode, Charset priorityCharset, int fnc1, SymbolShapeHint shape, int macroId) {
