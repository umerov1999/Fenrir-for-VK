...
public class MinimalECIInput implements ECIInput {
     ...
     *                                   {@code length()}
     * @throws IllegalArgumentException  if the value at the {@code index} argument is an ECI (@see #isECI)
     */
     ...
     * @throws IllegalArgumentException  if a value in the range {@code start}-{@code end} is an ECI (@see #isECI)
     */
     ...
     * @throws IndexOutOfBoundsException if the {@code index} argument is negative or not less than
     *                                   {@code length()}
     * @throws IllegalArgumentException  if the value at the {@code index} argument is not an ECI (@see #isECI)
     */
    ...
    private static final class InputEdge {
        final char c;
        final int encoderIndex; //the encoding of this edge
        final InputEdge previous;
        final int cachedTotalSize;

        InputEdge(char c, ECIEncoderSet encoderSet, int encoderIndex, InputEdge previous, int fnc1) {
