...
private fun SerialDescriptor.buildDeserializationNamesMap(json: Json): Map<String, Int> {
    ...
    for (i in 0 until elementsCount) {
        getElementAnnotations(i).filterIsInstance<JsonNames>()
            .singleOrNull()?.names?.forEach { name ->
        ...
        val nameToPut = when {
            ...
            strategyForClasses != null -> strategyForClasses.serialNameForJson(
                this,
                i,
                getElementName(i)
            )
...
internal fun Json.deserializationNamesMap(descriptor: SerialDescriptor): Map<String, Int> =
    ...
    ) { descriptor.buildDeserializationNamesMap(this) }

internal fun SerialDescriptor.serializationNamesIndices(
    json: Json,
...
internal fun SerialDescriptor.getJsonElementName(json: Json, index: Int): String {
    ...
    return if (strategy == null) getElementName(index) else serializationNamesIndices(
        json,
        strategy
    )[index]
}
